...
namespace ReactiveDomain.Foundation.Tests.Commands {
    ...
    public class when_using_command_tracker {
        ...
        [Fact]
        public void command_tracker_can_be_disposed() {
            var gotCmd = 0L;
            var gotTComplete = 0L;
            var gotCancel = 0L;
            var bus = new InMemoryBus("temp");
            ...
            AssertEx.IsOrBecomesTrue(() => Read(ref gotTComplete) == 0);
            AssertEx.IsOrBecomesTrue(() => Read(ref gotCancel) == 1);
            AssertEx.IsOrBecomesTrue(() => response is Canceled);
        }
        [Fact]
        public void publish_failure_will_fail_command() {
            var gotTComplete = 0L;
            var bus = new InMemoryBus("temp");
            bus.Subscribe(new AdHocHandler<TestCommands.Command1>(_ => throw new TestException()));
            bus.Subscribe(new AdHocHandler<CommandTracker.CommandProcessingCompleted>(_ => Increment(ref gotTComplete)));
            var cmd = new TestCommands.Command1();
            var t = new CommandTracker(
                cmd,
            ...
            AssertEx.IsOrBecomesTrue(() => Read(ref gotTComplete) == 1);
            AssertEx.IsOrBecomesTrue(() => (response as Fail)?.Exception is TestException);
        }
        [Fact]
        public void cannot_send_twice() {
            ...
            var done = 0L;
            var bus = new InMemoryBus("temp");
            bus.Subscribe(new AdHocHandler<TestCommands.Command1>(_ => SpinWait.SpinUntil(()=>Read(ref done) == 1)));
            bus.Subscribe(new AdHocHandler<CommandTracker.CommandProcessingCompleted>(_ => Increment(ref gotTComplete)));
            var cmd = new TestCommands.Command1();
            var t = new CommandTracker(
                cmd,
            ...
            AssertEx.IsOrBecomesTrue(() => Read(ref gotTComplete) == 1);
            AssertEx.IsOrBecomesTrue(() => (response as Fail)?.Exception is InvalidOperationException);
        }
